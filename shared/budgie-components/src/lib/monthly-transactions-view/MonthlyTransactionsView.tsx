...
export function MonthlyTransactionsView(props: MonthlyTransactionsViewProps) {
  ...
  const sendEmail = async (
    ...
    try {
      ...
      const storedProgressString = localStorage.getItem(accountKey);
      const storedProgress = storedProgressString
  ...
  return (
    <div className="w-full bg-[var(--main-background)] !z-0 min-h-full h-full portrait:h-[100vh]">
      ...
      {showPopup && selectedTransaction && (
        <div  className="fixed top-0 right-0 bottom-0 bg-black bg-opacity-50 flex justify-center items-center z-50000 w-[85vw] portrait:w-full p-2 text-sm md:text-lg lg:text-xl">
